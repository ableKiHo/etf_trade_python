            ...
            if sCode not in self.default_stock_list:
                ...
                if current_price > buy_price and profit_rate >= 1.8:
                    ...
                    if len(buy_after_rows) > 2 and thirdday_tic[self.customType.CURRENT_PRICE] > current_price and yesterday_tic[self.customType.CURRENT_PRICE] > current_price:
                        ...
                        if "half_sell_receipt" not in current_hold_stock and 20.0 >= profit_rate >= 10.0:
                            if current_hold_stock["half_sell"] is False:
                                ...
                                self.logging.logger.info("profit_10_sell_point check > [%s] >> %s / %s / %s" % (sCode, current_price, profit_rate, highest_profit_rate))
                                self.realtime_stop_loss_sell(sCode)

                        if ("half_sell_receipt" not in current_hold_stock) and (10.0 > profit_rate >= 3.0) and (current_hold_stock["half_sell"] is False):
                ...
                if current_price > buy_price and profit_rate >= 1.0:
                    ...
                    if yesterday_tic[self.customType.CURRENT_PRICE] > current_price:
                        ...
                        if 5.5 <= highest_profit_rate and highest_profit_rate > profit_rate:
                        ...
                        if 2.0 <= highest_profit_rate and highest_profit_rate > profit_rate and 1.45 < profit_rate <= 1.65:
                            self.logging.logger.info("inverse_highest_2.0_profit_sell_point check > [%s] >> %s / %s / %s" % (sCode, current_price, profit_rate, highest_profit_rate))
                            half_sell_limit_price = current_price - 50
                            self.realtime_stop_loss_limit_price_sell(sCode, half_sell_limit_price)

                    elif yesterday_tic[self.customType.CURRENT_PRICE] <= current_price:

                        if "half_sell_receipt" not in current_hold_stock and 1.6 <= profit_rate <= (highest_profit_rate - 0.3) and current_hold_stock["half_sell"] is False:
                            self.logging.logger.info("profit_std_half_sell_point check1 > [%s] >> %s / %s / %s" % (sCode, current_price, profit_rate, highest_profit_rate))
                            current_hold_stock["half_sell_receipt"] = True
                            self.realtime_stop_loss_half_sell(sCode)

                        if 5.5 < profit_rate <= (highest_profit_rate - 0.3):
