...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        if t[weekDay] == '토' or t[weekDay] == '일':
            self.line.notification("ETF AUTO TRADE END")
            QTest.qWait(5000)
            sys.exit()
        ...
        if 8 <= hour <= 10:
            ...
            for code in self.priority_portfolio_stock_dict.keys():
                screen_num = self.priority_portfolio_stock_dict[code]['스크린번호']
    ...
    def buyPriorityEtf(self, sCode, sRealType, sRealData):
        self.priority_cal_target_etf_stock_dict[sCode].update({"체결시간": self.total_cal_target_etf_stock_dict[sCode]["체결시간"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"현재가": self.total_cal_target_etf_stock_dict[sCode]["현재가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"전일대비": self.total_cal_target_etf_stock_dict[sCode]["전일대비"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"(최우선)매도호가": self.total_cal_target_etf_stock_dict[sCode]["(최우선)매도호가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"(최우선)매수호가": self.total_cal_target_etf_stock_dict[sCode]["(최우선)매수호가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"거래량": self.total_cal_target_etf_stock_dict[sCode]["거래량"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"누적거래량": self.total_cal_target_etf_stock_dict[sCode]["누적거래량"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"고가": self.total_cal_target_etf_stock_dict[sCode]["고가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"시가": self.total_cal_target_etf_stock_dict[sCode]["시가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"저가": self.total_cal_target_etf_stock_dict[sCode]["저가"]})
        self.priority_cal_target_etf_stock_dict[sCode].update({"당일시가": self.total_cal_target_etf_stock_dict[sCode]["당일시가"]})
        ...
        if goal_stock_price == 0:
            ...
            if quantity >= 1 and self.purchased_deposit > total_buy_price:
                ...
                self.priority_not_order_stock_dict.update({sCode: {"사유": "매수주문 불가 [주문가능 수량 부족]"}})
    ...
    def buySecondEtf(self, sCode, sRealType, sRealData):
        self.second_cal_target_etf_stock_dict[sCode].update({"체결시간": self.total_cal_target_etf_stock_dict[sCode]["체결시간"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"현재가": self.total_cal_target_etf_stock_dict[sCode]["현재가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"전일대비": self.total_cal_target_etf_stock_dict[sCode]["전일대비"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"(최우선)매도호가": self.total_cal_target_etf_stock_dict[sCode]["(최우선)매도호가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"(최우선)매수호가": self.total_cal_target_etf_stock_dict[sCode]["(최우선)매수호가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"거래량": self.total_cal_target_etf_stock_dict[sCode]["거래량"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"누적거래량": self.total_cal_target_etf_stock_dict[sCode]["누적거래량"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"고가": self.total_cal_target_etf_stock_dict[sCode]["고가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"시가": self.total_cal_target_etf_stock_dict[sCode]["시가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"저가": self.total_cal_target_etf_stock_dict[sCode]["저가"]})
        self.second_cal_target_etf_stock_dict[sCode].update({"당일시가": self.total_cal_target_etf_stock_dict[sCode]["당일시가"]})
        ...
        if goal_stock_price == 0:
            ...
            if quantity >= 1 and self.purchased_deposit > total_buy_price:
                ...
                self.logging.logger.info("[%s] > 매수주문 불가 [주문가능 수량 부족] [ 보유잔액: %s / 주문수량: %s / 총주문가: %s ]" % (sCode, self.purchased_deposit, quantity, total_buy_price))
                self.second_not_order_stock_dict.update({sCode: {"사유": "매수주문 불가 [주문가능 수량 부족]"}})
    ...
    def commRealData(self, sCode, sRealType, sRealData):
        ...
        self.total_cal_target_etf_stock_dict[sCode].update({"(최우선)매도호가": e})
        self.total_cal_target_etf_stock_dict[sCode].update({"(최우선)매수호가": f})
        self.total_cal_target_etf_stock_dict[sCode].update({"거래량": g})
        self.total_cal_target_etf_stock_dict[sCode].update({"누적거래량": h})
    ...
    def realdata_slot(self, sCode, sRealType, sRealData):
        if sRealType == "장시작시간":
            ...
            if value == '0':
                ...
                for code in self.priority_portfolio_stock_dict.keys():
                    self.dynamicCall("SetRealRemove(QString, QString)", self.priority_portfolio_stock_dict[code]['스크린번호'], code)
            ...
            for line in lines:
                if line != "":
                    ...
                    if stock_name.find("레버리지") or stock_name.find("인버스"):
                        self.priority_cal_target_etf_stock_dict.update({stock_code: {"종목명": stock_name,
                                                                                     "전일고가": highest_stock_price,
                                                                                     "전일저가": lowest_stock_price,
                                                                                     "전일종가": last_stock_price,
                                                                                     "목표가": ''}})
                    else:
                        self.second_cal_target_etf_stock_dict.update({stock_code: {"종목명": stock_name,
                                                                                   "전일고가": highest_stock_price,
                                                                                   "전일저가": lowest_stock_price,
                                                                                   "전일종가": last_stock_price,
                                                                                   "목표가": ''}})

                    self.total_portfolio_stock_dict.update({stock_code: {"종목명": stock_name,
                                                                                   "전일고가": highest_stock_price,
                                                                                   "전일저가": lowest_stock_price,
                                                                                   "전일종가": last_stock_price,
                                                                                   "목표가": ''}})
    ...
    def priority_screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            if code in self.priority_portfolio_stock_dict.keys():
                self.priority_portfolio_stock_dict[code].update({"스크린번호": str(temp_screen)})
                self.priority_portfolio_stock_dict[code].update({"주문용스크린번호": str(meme_screen)})
            elif code not in self.priority_portfolio_stock_dict.keys():
                self.priority_portfolio_stock_dict.update({code: {"스크린번호": str(temp_screen), "주문용스크린번호": str(meme_screen)}})

            cnt += 1

        self.logging.logger.info("매수용 > %s" % self.priority_portfolio_stock_dict)
    ...
    def second_screen_number_setting(self):
        ...
        for code in screen_overwrite:
            ...
            if code in self.second_portfolio_stock_dict.keys():
                self.second_portfolio_stock_dict[code].update({"스크린번호": str(temp_screen)})
                self.second_portfolio_stock_dict[code].update({"주문용스크린번호": str(meme_screen)})
            elif code not in self.second_portfolio_stock_dict.keys():
                self.second_portfolio_stock_dict.update({code: {"스크린번호": str(temp_screen), "주문용스크린번호": str(meme_screen)}})

            cnt += 1

        self.logging.logger.info("매수용(second) > %s" % self.second_portfolio_stock_dict)
